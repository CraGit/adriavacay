...
/**
 * @typedef {import("@prismicio/client").Content.HolidayHomesListSliceSlice} HolidayHomesListSliceSlice
 * @typedef {import("@prismicio/react").SliceComponentProps<HolidayHomesListSliceSlice>} HolidayHomesListSliceProps
 * @param {HolidayHomesListSliceProps}
 */
const HolidayHomesListSlice = async ({ slice }) => {
  ...
  // Filter to only cottages (holiday homes)
  const cottages = allAccommodations.filter(
    (a) => a.data.type === "Cottage"
  );

  const accommodationsWithCalendar = await Promise.all(
    cottages.map(async (a) => {
      try {
        if (locale === "de") {
...
export default HolidayHomesListSlice;
